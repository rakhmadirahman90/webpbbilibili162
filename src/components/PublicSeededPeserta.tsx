...
export default function PublicSeededPeserta(){
 ...
 const load=useCallback(async(forceLoading=false)=>{
  ...
  const task=(async()=>{
   if(forceLoading && playersRef.current.length===0 && mountedRef.current)setLoading(true);
   ...
   try{
    ...
    // Commit data sekali setelah seluruh halaman selesai agar tabel tidak berkedip.
