...
import { Search, Users, ShieldCheck, RefreshCw, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
export default function PublicSeededPeserta(){
 ...
 const filtered=useMemo(()=>{
  const q=norm(query);
  return q?players.filter(p=>norm([p.player_name,p.club_name,p.gender,p.seeded_quality].map(text).join(' ')).includes(q)):players;
 },[players,query]);
 ...
 const pageNumbers=useMemo<(number|'ellipsis')[]>(()=>{
  ...
  if(from>2)values.push('ellipsis');
  for(let n=from;n<=to;n++)values.push(n);
  if(to<totalPages-1)values.push('ellipsis');
