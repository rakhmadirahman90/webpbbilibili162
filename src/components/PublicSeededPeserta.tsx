...
export default function PublicSeededPeserta(){
 ...
 const pageNumbers=useMemo<(number|'ellipsis')[]>(()=>{
  ...
  const values:(number|'ellipsis')[]=[1]; const from=Math.max(2,safePage-1); const to=Math.min(totalPages-1,safePage+1);
  if(from>2)values.push('ellipsis'); for(let n=from;n<=to;n++)values.push(n); if(to<totalPages-1)values.push('ellipsis'); values.push(totalPages); return values;
 },[totalPages,safePage]);
 const goTo=(next:number)=>setPage(Math.max(1,Math.min(totalPages,next)));
 return <main className="min-h-screen w-full min-w-0 overflow-x-hidden bg-[#050b17] px-3 py-5 text-white sm:px-5 sm:py-8">
