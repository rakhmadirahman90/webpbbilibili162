  ...
  return (
    <div className="flex flex-col flex-grow min-h-0 w-full py-4 sm:py-6 space-y-4">
      ...
      <div className="flex-grow overflow-y-auto pr-1 custom-scrollbar pb-24">
        {loading ? <div className="text-center text-slate-500 py-20">Memuat inventaris…</div> : items.length === 0 ? <div className="flex flex-col items-center justify-center py-20 text-slate-500"><Box size={48} className="text-slate-600 mb-2" /><p className="text-sm font-bold uppercase tracking-wider">Belum Ada Data Inventaris</p></div> : (
            ...
            {items.map(item => (
              <div key={item.id} className="bg-[#0b1224] border border-white/5 rounded-2xl overflow-hidden relative group transition-all duration-300 hover:border-amber-500/20 hover:shadow-lg hover:shadow-amber-500/5 flex flex-col h-full">
                <div className="relative h-44 bg-black/40 overflow-hidden border-b border-white/5 shrink-0">
                  {item.gambar ? <LazyImage src={item.gambar} alt={item.nama} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" containerClassName="w-full h-full" width={400} /> : <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-amber-500/5 to-orange-500/10 text-amber-500/40"><Box size={36} className="stroke-[1.5]" /><span className="text-[10px] font-bold uppercase tracking-widest mt-2 text-slate-500">Tidak ada foto</span></div>}
