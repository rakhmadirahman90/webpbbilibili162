...
const getStoredPinData = (key: string): PinUserData | null => {
  try {
    ...
    if (!raw) return null;
    return JSON.parse(raw)[key.toLowerCase().trim()] || null;
  } catch { return null; }
};
...
export default function Login() {
  ...
  const setCleanPin = (value: string) => {
    setErrorMsg(null);
    setSuccessMsg(null);
    setPinInput(value.replace(/\D/g, '').slice(0, 12));
  };
  ...
  // One explicit, stable navigation contract for both admin and member login.
  ...
  const createMemberSession = (m: MemberRecord) => {
    ...
    return {
      user: {
        ...
        email: m.email || `${cleanName}@pbbilibili162.com`,
        user_metadata: {
          role: 'anggota', id: m.id, full_name: m.nama, nama: m.nama,
          whatsapp: m.whatsapp || '', kategori: m.kategori || m.kategori_atlet || 'SENIOR',
          kategori_atlet: m.kategori_atlet || m.kategori || 'SENIOR',
          jenis_kelamin: m.jenis_kelamin || 'Putra', domisili: m.domisili || 'PAREPARE',
          pengalaman: m.pengalaman || '', foto_url: m.foto_url || '', tanggal_lahir: m.tanggal_lahir || '',
          sektor_bermain: m.sektor_bermain || 'Tunggal & Ganda', ukuran_jersey: m.ukuran_jersey || 'L',
  ...
  const findMember = async (raw: string): Promise<MemberRecord | null> => {
    ...
    if (digits.length >= 6) {
      ...
      const match = byWa.data?.find((m: any) => {
        const d = String(m.whatsapp || '').replace(/[^0-9]/g, '');
        return d === digits || d.endsWith(digits);
      });
      if (match) return match as MemberRecord;
    }
    if (name.length >= 3) {
      const partial = await supabase.from('pendaftaran').select('*').ilike('nama', `%${name}%`).limit(10);
      const match = partial.data?.find((m: any) => {
        const n = String(m.nama || '').trim().toLowerCase();
      ...
      <main className="relative z-10 w-full max-w-[440px]">
        <div className="rounded-[30px] border border-white/[0.10] bg-[#0a1121]/95 backdrop-blur-2xl shadow-[0_30px_90px_rgba(0,0,0,.55)] overflow-hidden">
          ...
          <div className="p-5 sm:p-7">
            ...
            {errorMsg && (
              <div className="mt-5 rounded-2xl border border-red-400/20 bg-red-500/[0.07] p-3.5 flex gap-3" role="alert">
                <AlertCircle size={17} className="mt-0.5 shrink-0 text-red-400" />
                <div><p className="text-xs font-extrabold text-red-300">Akses Ditolak</p><p className="mt-0.5 text-[11px] leading-relaxed text-red-200/70">{errorMsg}</p></div>
              </div>
            )}
            {successMsg && (
              <div className="mt-5 rounded-2xl border border-emerald-400/20 bg-emerald-500/[0.07] p-3.5 flex gap-3" role="status">
                <CheckCircle2 size={17} className="mt-0.5 shrink-0 text-emerald-400" />
                <p className="text-[11px] leading-relaxed text-emerald-200/80">{successMsg}</p>
              </div>
            )}

            <form onSubmit={(e) => { e.preventDefault(); void verifyAndLogin(); }} className="mt-5 space-y-4">
              <div>
                ...
                <div className="relative">
                  <input type="text" required autoComplete="username" value={usernameInput} onChange={(e) => { setErrorMsg(null); setSuccessMsg(null); setUsernameInput(e.target.value); }} className={`${input} pl-4 pr-11 text-sm font-semibold placeholder:text-slate-600`} placeholder="Nama anggota / WhatsApp / admin" />
                  <User size={17} className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-slate-600" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <input type={showPin ? 'text' : 'password'} inputMode="numeric" autoComplete="current-password" value={pinInput} onChange={(e) => setCleanPin(e.target.value)} className={`${input} pl-4 pr-12 text-center font-mono text-lg tracking-[0.32em] placeholder:text-slate-700 placeholder:tracking-normal placeholder:text-xs`} placeholder="Masukkan PIN" />
                  <button type="button" onClick={() => setShowPin(v => !v)} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-xl p-2 text-slate-500 transition hover:bg-white/5 hover:text-white" aria-label={showPin ? 'Sembunyikan PIN' : 'Tampilkan PIN'}>{showPin ? <EyeOff size={17} /> : <Eye size={17} />}</button>
                </div>
              </div>
              ...
              <button type="submit" disabled={loading} className="group relative flex h-14 w-full items-center justify-center gap-2.5 overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 to-blue-500 text-sm font-black uppercase tracking-[0.16em] text-white shadow-[0_12px_30px_rgba(37,99,235,.25)] transition-all duration-200 hover:-translate-y-0.5 hover:from-blue-500 hover:to-cyan-500 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60">
                ...
                <span>{loading ? 'Memverifikasi…' : 'Masuk Portal'}</span>
              </button>
            </form>
