...
export default function AdminPopup() {
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const { data, error } = await supabase.from('konfigurasi_popup').update({ judul: form.judul.trim(), deskripsi: form.deskripsi.trim(), url_gambar: form.url_gambar.trim(), file_url: form.file_url.trim() || null }).eq('id', editingId).select('*').single();
        if (error) throw error;
        if (!data) throw new Error('Supabase tidak mengembalikan data setelah update.');
      } else {
        const { data, error } = await supabase.rpc('pb162_popup_create', { p_url_gambar: form.url_gambar.trim(), p_judul: form.judul.trim(), p_deskripsi: form.deskripsi.trim(), p_file_url: form.file_url.trim() || null });
        if (error) throw error;
        if (!data) throw new Error('Supabase tidak mengembalikan pop-up baru.');
      }
      await loadPopups();
      resetForm();
      await Swal.fire({ title: 'Berhasil', text: wasEditing ? 'Pop-up diperbarui dan urutannya dipertahankan.' : 'Pop-up baru menjadi URUTAN 1. Pop-up sebelumnya otomatis bergeser ke urutan berikutnya.', icon: 'success', background: '#0F172A', color: '#fff' });
    } catch (error: any) {
  ...
  const movePopup = async (item: PopupConfig, direction: 'up' | 'down') => {
    setMovingId(item.id);
    try {
      const { error } = await supabase.rpc('pb162_popup_move', { p_id: item.id, p_direction: direction });
      if (error) throw error;
      await loadPopups();
    } catch (error: any) {
      Swal.fire('Gagal mengubah urutan', error?.message || 'Urutan pop-up tidak dapat diubah.', 'error');
  ...
  const togglePopup = async (item: PopupConfig) => {
    try {
      const { error } = await supabase.from('konfigurasi_popup').update({ is_active: !item.is_active }).eq('id', item.id);
      if (error) throw error;
      await loadPopups();
  ...
  const deletePopup = async (item: PopupConfig) => {
    ...
    try {
      const { error } = await supabase.rpc('pb162_popup_delete', { p_id: item.id });
      if (error) throw error;
      await loadPopups();
