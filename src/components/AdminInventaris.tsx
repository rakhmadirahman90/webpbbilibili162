...
export default function AdminInventaris() {
  ...
  const fetchItems = async () => {
    try {
      ...
      setItems(rows);
      localStorage.setItem('inventaris_local_v3', JSON.stringify(rows));
    } catch (error) {
      console.error('Gagal mengambil inventaris dari Supabase:', error);
      try {
        const cached = JSON.parse(localStorage.getItem('inventaris_local_v3') || '[]');
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('inventaris-admin-realtime')
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      void Swal.fire({ icon: 'error', title: 'File terlalu besar', text: 'Ukuran foto maksimal 2MB.' });
      return;
    }
    const reader = new FileReader();
    reader.onload = () => setFormData(prev => ({ ...prev, gambar: String(reader.result || '') }));
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        ...
        const { error } = await supabase.from('inventaris').insert({ id, ...payload });
        if (error) throw error;
      }
      setShowModal(false);
      await fetchItems();
      ...
      void Swal.fire({ icon: 'error', title: 'Gagal menyimpan', text: error?.message || 'Perubahan tidak dapat disimpan ke Supabase.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#0b1224] p-6 rounded-3xl border border-white/5 relative overflow-hidden">
        ...
        <div className="relative z-10"><h1 className="text-2xl sm:text-3xl font-black text-white italic uppercase tracking-tight flex items-center gap-3"><PackageOpen className="text-amber-500" size={28} />Inventaris Klub</h1><p className="text-slate-400 text-sm mt-1">Supabase sebagai sumber data utama • realtime aktif</p></div>
        <button onClick={openAdd} className="relative z-10 inline-flex items-center justify-center gap-2 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white text-sm font-black uppercase tracking-wider px-5 py-3 rounded-2xl shadow-lg shadow-amber-600/30 active:scale-95 transition-all"><Plus size={18} />Tambah Barang</button>
      </div>
