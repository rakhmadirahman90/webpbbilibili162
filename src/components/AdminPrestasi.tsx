...
import { Medal, Plus, Edit, Trash2, Trophy } from 'lucide-react';
...
export default function AdminPrestasi() {
  ...
  const handleOpenEdit = (item: Prestasi) => { setEditingItem(item); setFormData(item); setShowModal(true); };

  const handleDelete = async (id: string) => {
    const result = await Swal.fire({ title: 'Hapus Prestasi?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: 'Ya, Hapus!' });
    if (result.isConfirmed) {
      try {
        const { error } = await supabase.from('prestasi_klub').delete().eq('id', id);
        if (error) throw error;
        setPrestasi(prestasi.filter(i => i.id !== id));
      } catch (error: any) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchPrestasi(); setShowModal(false);
      Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Tersimpan', showConfirmButton: false, timer: 1500 });
    } catch (error: any) {
      let updated;
      if (editingItem) updated = prestasi.map(i => i.id === editingItem.id ? { ...i, ...formData } : i);
      else updated = [{ ...formData, id: 'p_' + Date.now() }, ...prestasi];
      localStorage.setItem('prestasi_local_v3', JSON.stringify(updated));
      setPrestasi(updated); setShowModal(false);
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#0b1224] p-6 rounded-3xl border border-white/5 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-2xl sm:text-3xl font-black text-white italic uppercase tracking-tight flex items-center gap-3">
            <Trophy className="text-yellow-500" size={28} /> Prestasi Klub
          </h1>
          <p className="text-slate-400 text-sm mt-1">Kelola data prestasi dan kejuaraan (Tampil di Landing Page)</p>
        </div>
        <button onClick={handleOpenAdd} className="relative z-10 inline-flex items-center justify-center gap-2 bg-gradient-to-r from-yellow-600 to-amber-600 hover:from-yellow-500 hover:to-amber-500 text-white text-sm font-black uppercase tracking-wider px-5 py-3 rounded-2xl shadow-lg shadow-yellow-600/30 active:scale-95 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {prestasi.map(item => (
          <div key={item.id} className="bg-[#0b1224] border border-white/5 rounded-2xl p-5 relative group overflow-hidden flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1">
                <h3 className="font-bold text-white text-lg leading-tight">{item.nama_kejuaraan}</h3>
                <p className="text-xs text-blue-400 font-medium mb-3">Tingkat {item.tingkat}</p>
                <div className="flex gap-3 mb-3">
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-yellow-500/10 rounded-lg border border-yellow-500/20"><Medal size={14} className="text-yellow-500" /><span className="text-xs font-bold text-yellow-500">{item.medali_emas}</span></div>
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-slate-400/10 rounded-lg border border-slate-400/20"><Medal size={14} className="text-slate-400" /><span className="text-xs font-bold text-slate-400">{item.medali_perak}</span></div>
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-amber-700/10 rounded-lg border border-amber-700/20"><Medal size={14} className="text-amber-600" /><span className="text-xs font-bold text-amber-600">{item.medali_perunggu}</span></div>
                </div>
                <div className="bg-black/30 p-2 rounded-lg border border-white/5">
                    <span className="text-[10px] text-slate-500 uppercase block mb-1">Atlet Berprestasi:</span>
                    <span className="text-xs text-slate-300">{item.atlet_berprestasi || '-'}</span>
                </div>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#0b1224] border border-white/10 rounded-3xl p-6 w-full max-w-lg shadow-2xl">
            <h3 className="text-lg font-black text-white uppercase italic mb-4">{editingItem ? 'Edit Prestasi' : 'Tambah Prestasi'}</h3>
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2"><label className="text-xs font-bold text-slate-400 mb-1 block">Nama Kejuaraan</label><input required type="text" value={formData.nama_kejuaraan} onChange={e => setFormData({...formData, nama_kejuaraan: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none" /></div>
                <div><label className="text-xs font-bold text-slate-400 mb-1 block">Tingkat</label><select value={formData.tingkat} onChange={e => setFormData({...formData, tingkat: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none"><option>Kabupaten/Kota</option><option>Provinsi</option><option>Nasional</option><option>Internasional</option></select></div>
                <div><label className="text-xs font-bold text-slate-400 mb-1 block">Tahun</label><input required type="number" value={formData.tahun} onChange={e => setFormData({...formData, tahun: parseInt(e.target.value)})} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none" /></div>
                
                <div className="col-span-2 grid grid-cols-3 gap-2">
                    <div><label className="text-xs font-bold text-yellow-500 mb-1 block">Emas</label><input required type="number" min="0" value={formData.medali_emas} onChange={e => setFormData({...formData, medali_emas: parseInt(e.target.value)})} className="w-full bg-yellow-500/10 border border-yellow-500/30 rounded-xl px-3 py-2 text-yellow-500 outline-none" /></div>
                    <div><label className="text-xs font-bold text-slate-400 mb-1 block">Perak</label><input required type="number" min="0" value={formData.medali_perak} onChange={e => setFormData({...formData, medali_perak: parseInt(e.target.value)})} className="w-full bg-slate-500/10 border border-slate-500/30 rounded-xl px-3 py-2 text-slate-300 outline-none" /></div>
                    <div><label className="text-xs font-bold text-amber-600 mb-1 block">Perunggu</label><input required type="number" min="0" value={formData.medali_perunggu} onChange={e => setFormData({...formData, medali_perunggu: parseInt(e.target.value)})} className="w-full bg-amber-700/10 border border-amber-700/30 rounded-xl px-3 py-2 text-amber-600 outline-none" /></div>
                </div>
                
                <div className="col-span-2"><label className="text-xs font-bold text-slate-400 mb-1 block">Daftar Atlet Berprestasi</label><textarea value={formData.atlet_berprestasi} onChange={e => setFormData({...formData, atlet_berprestasi: e.target.value})} placeholder="Pisahkan dengan koma (Ahmad, Budi, dll)" className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none min-h-[80px]" /></div>
              </div>
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className="flex-1 py-3 rounded-xl font-bold text-white bg-yellow-600 hover:bg-yellow-500">Simpan</button>
              </div>
            </form>
          </div>
        </div>
