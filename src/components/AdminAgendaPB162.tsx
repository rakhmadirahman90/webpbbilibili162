...
import { CalendarDays, Clock3, MapPin, Pencil, Plus, Radio, Save, Trash2, X } from 'lucide-react';
import Swal from 'sweetalert2';
import { supabase } from '../supabase';

type AgendaItem = { id:string; title:string; description:string|null; event_date:string; start_time:string|null; end_time:string|null; location:string|null; category:string|null; status:string|null; is_published:boolean };
type FormState = Omit<AgendaItem,'id'>;
const empty:FormState={title:'',description:'',event_date:'',start_time:'',end_time:'',location:'',category:'Kegiatan Klub',status:'Terjadwal',is_published:true};

export default function AdminAgendaPB162(){
 const [items,setItems]=useState<AgendaItem[]>([]); const [form,setForm]=useState<FormState>(empty); const [editing,setEditing]=useState<string|null>(null); const [loading,setLoading]=useState(true); const [saving,setSaving]=useState(false); const [live,setLive]=useState(false);
 const load=useCallback(async()=>{const {data,error}=await supabase.from('agenda_pb162').select('*').order('event_date',{ascending:true}).order('start_time',{ascending:true});if(!error)setItems((data||[]) as AgendaItem[]);setLoading(false)},[]);
 useEffect(()=>{void load();const ch=supabase.channel(`agenda-admin-${Date.now()}`).on('postgres_changes',{event:'*',schema:'public',table:'agenda_pb162'},p=>{const row=(p.new||p.old) as AgendaItem;setItems(cur=>{if(p.eventType==='DELETE')return cur.filter(x=>x.id!==row.id);const next=cur.some(x=>x.id===row.id)?cur.map(x=>x.id===row.id?row:x):[...cur,row];return next.sort((a,b)=>`${a.event_date}${a.start_time||''}`.localeCompare(`${b.event_date}${b.start_time||''}`))})}).subscribe(s=>setLive(s==='SUBSCRIBED'));return()=>{supabase.removeChannel(ch)}},[load]);
