...
type GalleryItem = { id: string; type: 'image' | 'video'; url: string; title?: string; category?: string; description?: string; created_at?: string; updated_at?: string; thumbnail_url?: string; };

const splitUrls = (value?: string) => value ? value.split(/[\s,]+/).map(url => url.trim()).filter(url => /^https?:\/\//i.test(url) || url.startsWith('/')) : [];

function sameGallery(a: GalleryItem[], b: GalleryItem[]) {
  if (a.length !== b.length) return false;
  return a.every((item, index) => { const next = b[index]; return next && `${item.id}|${item.type}|${item.url}|${item.thumbnail_url || ''}|${item.updated_at || item.created_at || ''}` === `${next.id}|${next.type}|${next.url}|${next.thumbnail_url || ''}|${next.updated_at || next.created_at || ''}`; });
}
...
export default function Gallery() {
  ...
  useEffect(() => { try { const saved = localStorage.getItem('pb_us_liked_gallery'); if (saved) setLikedItems(new Set(JSON.parse(saved))); } catch {} }, []);
  useEffect(() => setCurrentPage(1), [activeTab, searchQuery]);
  useEffect(() => { window.dispatchEvent(new CustomEvent(selectedId ? 'pb-overlay-open' : 'pb-overlay-close')); return () => window.dispatchEvent(new CustomEvent('pb-overlay-close')); }, [selectedId]);
  ...
  const fetchGallery = useCallback(async (initial = false) => {
    ...
    try {
      ...
      if (!sbError && Array.isArray(sbData) && sbData.length > 0) { applyGallery(sbData as GalleryItem[]); try { localStorage.setItem(CACHE_KEY, JSON.stringify(sbData)); } catch {} return; }
      const legacy = await getSiteSetting('gallery_list');
      if (Array.isArray(legacy) && legacy.length > 0) { applyGallery(legacy as GalleryItem[]); try { localStorage.setItem(CACHE_KEY, JSON.stringify(legacy)); } catch {} return; }
      if (initial) {
        try { const cached = localStorage.getItem(CACHE_KEY) || localStorage.getItem(LEGACY_CACHE_KEY); if (cached) { const parsed = JSON.parse(cached); if (Array.isArray(parsed) && parsed.length > 0) { applyGallery(parsed as GalleryItem[]); return; } } } catch {}
    ...
    } finally { fetchInFlight.current = false; if (initial) setLoading(false); }
  }, [applyGallery]);
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('app_data_changed', handleUpdate); window.removeEventListener('table_updated_gallery', handleUpdate); window.removeEventListener('site_setting_updated', handleUpdate); supabase.removeChannel(channel); };
  }, [fetchGallery]);
  ...
  useEffect(() => {
    ...
    if (urlId) { const found = galleryItems.find(item => item.id === urlId); if (found) { setActiveTab(found.type); setSelectedId(found.id); setActiveImgIndex(0); } }
  ...
  useEffect(() => {
    ...
    const urlId = searchParams.get('gallery') || searchParams.get('galleryId') || searchParams.get('photoId') || searchParams.get('videoId');
    if (selectedId && searchParams.get('gallery') !== selectedId) setSearchParams(prev => { const next = new URLSearchParams(prev); next.set('gallery', selectedId); ['galleryId', 'photoId', 'videoId'].forEach(key => next.delete(key)); return next; }, { replace: true });
    else if (!selectedId && urlId) setSearchParams(prev => { const next = new URLSearchParams(prev); ['gallery', 'galleryId', 'photoId', 'videoId'].forEach(key => next.delete(key)); return next; }, { replace: true });
  }, [selectedId, searchParams, setSearchParams, urlInitialized]);

  useEffect(() => { const handleEsc = (event: KeyboardEvent) => { if (event.key === 'Escape') setSelectedId(null); }; window.addEventListener('keydown', handleEsc); return () => window.removeEventListener('keydown', handleEsc); }, []);

  const getYouTubeID = useCallback((url: string) => { if (!url) return null; const match = url.match(/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|&v=|shorts\/)([^#&?]*).*/); return match && match[2]?.length === 11 ? match[2] : null; }, []);
  const getThumbnail = useCallback((item: GalleryItem) => { if (!item.url) return '/placeholder-image.jpg'; if (item.type === 'image') return splitUrls(item.url)[0] || '/placeholder-image.jpg'; const videoId = getYouTubeID(item.url); if (videoId) return `https://img.youtube.com/vi/${videoId}/hqdefault.jpg`; return item.thumbnail_url || ''; }, [getYouTubeID]);
  const getGalleryImages = useCallback((item: GalleryItem) => item?.type === 'image' ? splitUrls(item.url) : [], []);

  const filteredMedia = useMemo(() => { const query = searchQuery.toLowerCase().trim(); return galleryItems.filter(item => { if (item.type !== activeTab) return false; if (!query) return true; return item.title?.toLowerCase().includes(query) || item.category?.toLowerCase().includes(query); }); }, [galleryItems, activeTab, searchQuery]);
    ...
    <section id="gallery" className="bg-[#f8fafc] pb-24 pt-10 md:pt-14 gallery-stable">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-5 mb-8 md:mb-12">
          ...
          <div className="relative w-full md:max-w-xs lg:max-w-md"><div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400"><Search size={16} /></div><input type="text" value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder={`Cari ${activeTab === 'image' ? 'foto' : 'video'} galeri...`} className="w-full pl-11 pr-11 py-3 bg-white border border-slate-200 rounded-full text-xs font-semibold text-slate-700 placeholder-slate-400 focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 shadow-xs" />{searchQuery && <button onClick={() => setSearchQuery('')} className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400" aria-label="Hapus pencarian"><X size={15} /></button>}</div>
        </div>

        {loading ? <div className="flex flex-col items-center justify-center py-32 text-slate-500"><Loader2 className="mb-4 text-emerald-500" size={40} /><p className="font-bold uppercase tracking-widest text-[10px]">Sinkronisasi Galeri...</p></div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {paginatedMedia.length > 0 ? paginatedMedia.map(item => { const thumbnail = getThumbnail(item); const photoCount = getGalleryImages(item).length; return <article key={item.id} onClick={() => { setSelectedId(item.id); setActiveImgIndex(0); }} className="gallery-card group relative cursor-pointer overflow-hidden rounded-xl bg-white border border-slate-100 flex flex-col shadow-xs">
            <div className="aspect-[1.5/1] relative overflow-hidden bg-slate-100 shrink-0">
              {item.type === 'video' && !getYouTubeID(item.url) && thumbnail ? <video src={`${item.url}#t=0.5`} poster={thumbnail} muted playsInline preload="metadata" className="w-full h-full object-cover block" /> : thumbnail ? <LazyImage src={thumbnail} alt={item.title || ''} containerClassName="w-full h-full" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-400"><PlayCircle size={42} /></div>}
        ...
        {activeMedia && (() => {
          const images = getGalleryImages(activeMedia); const count = images.length; const currentImage = images[activeImgIndex] || images[0] || '';
          return <div className="gallery-lightbox fixed inset-0 z-[110000] bg-white text-slate-900 h-[100dvh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 min-h-0 w-full flex flex-col overflow-hidden bg-white">
              <div className="shrink-0 w-full bg-[#030712] relative h-[38vh] sm:h-[48vh] md:h-[58vh] lg:h-[65vh] overflow-hidden flex items-center justify-center border-b border-slate-900/40">
                {activeMedia.type === 'video' ? <div className="w-full h-full flex items-center justify-center bg-black">{getYouTubeID(activeMedia.url) ? <iframe key={activeMedia.id} className="w-full h-full max-h-screen border-0" src={`https://www.youtube.com/embed/${getYouTubeID(activeMedia.url)}?autoplay=1&rel=0`} title={activeMedia.title || 'Video galeri'} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /> : <video key={activeMedia.id} src={activeMedia.url} className="w-full h-full max-h-screen object-contain" controls autoPlay playsInline preload="metadata" />}</div> : <div className="relative w-full h-full flex items-center justify-center bg-[#030712]">{currentImage && <img key={`${activeMedia.id}-${activeImgIndex}`} src={getOptimizedImageUrl(currentImage, 1400)} alt={`${activeMedia.title || 'Foto'} ${activeImgIndex + 1}`} loading="eager" decoding="async" className="max-w-full max-h-full object-contain block" referrerPolicy="no-referrer" />}{count > 1 && <><button onClick={() => goToImage(activeImgIndex - 1, count)} className="absolute left-3.5 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/65 hover:bg-black/80 text-white z-20" aria-label="Foto sebelumnya"><ChevronLeft size={22} /></button><button onClick={() => goToImage(activeImgIndex + 1, count)} className="absolute right-3.5 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/65 hover:bg-black/80 text-white z-20" aria-label="Foto berikutnya"><ChevronRight size={22} /></button></>}{count > 0 && <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-black/70 text-white px-3 py-1.5 rounded-full text-[11px] font-black tracking-wider z-20">FOTO {activeImgIndex + 1} / {count}</div>}</div>}
              </div>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain touch-pan-y">
                ...
                {activeMedia.type === 'image' && count > 1 && <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-2 pb-8">
                  <div className="flex items-center justify-between gap-3 mb-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Album Foto</p><p className="text-sm font-extrabold text-slate-800">{count} foto dokumentasi</p></div><span className="text-[10px] font-black uppercase tracking-wider text-emerald-600">Foto {activeImgIndex + 1} dipilih</span></div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 pb-2">{images.map((img, idx) => <button key={`${activeMedia.id}-grid-${idx}`} onClick={() => setActiveImgIndex(idx)} className={`group relative aspect-[4/3] overflow-hidden rounded-xl bg-slate-100 border-2 transition-all focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 ${idx === activeImgIndex ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-transparent hover:border-slate-300'}`} aria-label={`Buka foto ${idx + 1}`}><img src={getOptimizedImageUrl(img, 520)} alt={`Foto ${idx + 1} dari ${count}`} loading="lazy" decoding="async" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" referrerPolicy="no-referrer" /><span className="absolute left-2 bottom-2 min-w-7 h-7 px-2 rounded-full bg-black/75 text-white text-[10px] font-black flex items-center justify-center">{idx + 1}</span>{idx === activeImgIndex && <span className="absolute top-2 right-2 bg-emerald-500 text-white px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider">Terpilih</span>}</button>)}</div>
                </div>}
              </div>
            </div>
          </div>;
