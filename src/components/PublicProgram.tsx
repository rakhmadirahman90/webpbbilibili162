  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        ...
        console.error('[program] fresh data load failed', e);
        setPrograms([]);
      }
    };
    fetchPrograms();
    const channel = supabase.channel('public-program-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'program_latihan' }, () => fetchPrograms())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'site_settings', filter: 'key=eq.program_list' }, () => fetchPrograms())
      .subscribe();
