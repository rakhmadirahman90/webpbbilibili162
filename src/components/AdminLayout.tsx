...
export default function AdminLayout({ children, email }: AdminLayoutProps) {
  ...
  // Keep the page itself stable while the mobile drawer is open so taps/scrolls
  // cannot leak through to the content underneath it.
  useEffect(() => {
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 h-[100dvh] overflow-hidden">
        <header className="md:hidden sticky top-0 flex-shrink-0 px-4 py-3 bg-white/95 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between z-40 shadow-sm">
          <button
            ...
            className="min-w-11 min-h-11 p-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 rounded-xl transition-colors flex items-center gap-2 touch-manipulation"
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 px-2.5 py-1.5 rounded-full whitespace-nowrap">
            Admin Portal
          </span>
        </header>
