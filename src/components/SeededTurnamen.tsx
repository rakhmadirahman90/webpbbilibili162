...
import { SEED_OPTIONS, syncLatestTournamentSeedsToMembers } from '../utils/seededSync';
...
export default function SeededTurnamen() {
  ...
  const savePlayer=async(e:React.FormEvent)=>{
    ...
    try {
      ...
      await syncLatestTournamentSeedsToMembers();
  ...
  const deletePlayer=async(p:Player)=>{
    ...
    try{const {error:e}=await supabase.from('seeded_players').delete().eq('id',p.id);if(e)throw e;await syncLatestTournamentSeedsToMembers();await Swal.fire({icon:'success',title:'Pemain dihapus',timer:1000,showConfirmButton:false,background:'#0b1324',color:'#fff'});await loadPlayers();}
