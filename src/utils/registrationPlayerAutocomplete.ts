...
async function loadPlayers(): Promise<PlayerRecord[]> {
  ...
  const addSeeded = (row: any) => {
    const name = clean(row.player_name); const club = clean(row.club_name); const seeded = clean(row.seeded_quality); const level = clean(row.division_level); const category = clean(row.eligible_category);
  ...
  const addRegistered = (row: any) => {
    const name = clean(row.nama); const club = clean(row.asal_pb); const category = clean(row.kategori_atlet || row.kategori); const key = [norm(name), norm(club), '', '', norm(category)].join('::');
    if (!norm(name) || map.has(key)) return;
...
function findInputs(): HTMLInputElement[] {
  const labels = Array.from(document.querySelectorAll('label')); const result: HTMLInputElement[] = [];
...
function attach(input: HTMLInputElement, players: PlayerRecord[], index: number) {
  ...
  const wrapper = document.createElement('div'); wrapper.style.cssText = 'position:relative;width:100%;min-width:0;z-index:20;box-sizing:border-box;'; input.parentElement?.insertBefore(wrapper, input); wrapper.appendChild(input);
  const status = document.createElement('div'); status.dataset.pbPlayerStatus = '1'; status.style.cssText = 'min-height:18px;margin-top:5px;padding:0 2px;font:700 10px/1.35 system-ui,sans-serif;overflow-wrap:anywhere;'; wrapper.appendChild(status);
  const dropdown = document.createElement('div'); dropdown.style.cssText = 'display:none;position:relative;left:auto;right:auto;top:auto;width:100%;max-width:100%;max-height:280px;overflow-x:hidden;overflow-y:auto;box-sizing:border-box;margin-top:6px;background:#071225;border:1px solid rgba(96,165,250,.28);border-radius:14px;box-shadow:0 12px 30px rgba(0,0,0,.38);padding:5px;z-index:30;-webkit-overflow-scrolling:touch;'; wrapper.appendChild(dropdown);
  ...
  const render = () => {
    ...
    matches.forEach(player => {
      const button = document.createElement('button'); button.type = 'button'; button.style.cssText = 'display:block;width:100%;max-width:100%;box-sizing:border-box;text-align:left;padding:10px 11px;margin:0;border:0;border-radius:10px;background:transparent;color:#fff;cursor:pointer;touch-action:manipulation;overflow:hidden;';
      ...
      button.addEventListener('mouseenter', () => { button.style.background = 'rgba(37,99,235,.16)'; }); button.addEventListener('mouseleave', () => { button.style.background = 'transparent'; }); button.addEventListener('pointerdown', event => event.preventDefault());
  ...
  input.addEventListener('input', () => { selectedKey = ''; render(); }); input.addEventListener('focus', () => { active = true; if (input.value.trim()) render(); });
  ...
  document.addEventListener('pointerdown', event => { if (active && !wrapper.contains(event.target as Node)) dropdown.style.display = 'none'; }); input.dataset.pbPlayerIndex = String(index + 1);
}
...
// Reliability guard: serialize tournament document uploads and retry transient
// mobile/network failures so concurrent multipart requests do not surface as
// a generic "Failed to fetch" in the public registration form.
      ...
      const run = async () => {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
          try {
            ...
            if (result?.error && isTransientUploadError(result.error) && attempt < 2) { lastError = result.error; await sleep(700 * (attempt + 1)); continue; }
            return result;
          } catch (error) {
            lastError = error;
