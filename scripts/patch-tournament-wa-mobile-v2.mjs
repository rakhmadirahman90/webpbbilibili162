...
const newActions = `function Actions({onDetail,onEdit,onPayment,onAccept,onReject,onDelete,paymentVerified,registrationPending,full=false,row}:{onDetail:()=>void,onEdit:()=>void,onPayment:()=>void,onAccept:()=>void,onReject:()=>void,onDelete:()=>void,paymentVerified:boolean,registrationPending:boolean,full?:boolean,row:Registration}){
  return <div className={'flex ' + (full ? 'w-full ' : '') + 'flex-wrap gap-1.5'}>
    ...
    {!registrationPending&&<button type="button" title="Kirim konfirmasi status melalui WhatsApp" aria-label="Kirim konfirmasi status melalui WhatsApp" onClick={()=>void openTournamentWhatsApp(row)} className="inline-flex min-h-9 items-center justify-center gap-1.5 rounded-lg border border-green-200 bg-green-50 px-2.5 text-[10px] font-black text-green-700 hover:bg-green-100"><MessageCircle size={14}/> WA</button>}
