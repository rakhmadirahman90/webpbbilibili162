...
if (!source.includes('POPUP_SWIPE_V4')) {
  ...
  source = source.replace(refTarget, `${refTarget}\n\n  // POPUP_SWIPE_V4: horizontal pointer swipe on the entire popup card.\n  const swipeRef = useRef({ pointerId: null as number | null, startX: 0, startY: 0, active: false, horizontal: false });`);
}
...
const legacyHandler = `  const handleDragEnd = (e: any, { offset, velocity }: any) => {\n    const swipe = Math.abs(offset.x) > 50; \n    if (swipe) {\n      if (offset.x < 0) {\n        setCurrentIndex((prev) => (prev + 1) % promoImages.length);\n      } else {\n        setCurrentIndex((prev) => (prev - 1 + promoImages.length) % promoImages.length);\n      }\n    }\n  };`;

if (source.includes('const changePopupBySwipe')) {
  // Already patched: do not modify again.
} else if (source.includes(legacyHandler)) {
  source = source.replace(legacyHandler, dragReplacement);
} else {
  throw new Error('[popup-swipe-v4] legacy swipe handler not found in expected source');
}

if (!source.includes('onPointerDown={handleSwipePointerDown}')) {
  const cardMarker = '          onClick={(e) => e.stopPropagation()}';
  if (!source.includes(cardMarker)) throw new Error('[popup-swipe-v4] popup card click target not found');
...
// Remove the old Framer Motion horizontal drag wrapper while preserving its inner content.
