...
`;
  s = s.replace(
    ...
    helper + "export default function SeededTurnamen()"
  );
}
...
if (!s.includes('const exportPDF = async')) {
  ...
  const exportPDF = async () => {
    if (!exportRows.length) {
      return void Swal.fire({
        ...
        text: 'Tidak ada data seeded untuk diekspor.',
        background: '#0b1324',
        color: '#fff'
      });
    }
    ...
    const countLabel = String(exportRows.length) + ' pemain • No, Nama Pemain, Klub & Seeded';

    doc.setProperties({
      title: 'Seeded Pemain — PB BILIBILI 162',
      subject: 'Daftar seeded pemain'
    });
    doc.setFillColor(8, 21, 48);
    doc.rect(0, 0, pageW, 30, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.text('SEEDED PEMAIN — PB BILIBILI 162', 15, 12);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.text(countLabel, 15, 19);
    doc.setFontSize(7.5);
    doc.text('Dicetak ' + exportDate(), 15, 25);
    ...
    autoTable(doc, {
      ...
      body: exportRows.map((p, i) => [
        i + 1,
        text(p.player_name) || '—',
        text(p.club_name) || '—',
        text(p.seeded_quality) || '—'
      ]),
      theme: 'grid',
      styles: {
        font: 'helvetica',
      ...
      didDrawPage: (data: any) => {
        ...
        doc.text(
          'PB BILIBILI 162 • Halaman ' + String(data.pageNumber),
          pageW / 2,
          pageH - 6,
          { align: 'center' }
        );
    ...
    doc.save(
      'Seeded_Pemain_PB_BILIBILI_162_' +
  ...
  const exportExcel = () => {
    ...
    const rows = [
      ...
      [String(exportRows.length) + ' pemain', '', '', ''],
      ['Dicetak ' + exportDate(), '', '', ''],
      [],
    ...
    ws['!autofilter'] = { ref: 'A5:D' + String(exportRows.length + 5) };
    ws['!pageSetup'] = {
    ...
    ws['!ref'] = 'A1:D' + String(exportRows.length + 5);

    const safe = safeFileName(
      'Seeded_Pemain_PB_BILIBILI_162_' + new Date().toISOString().slice(0, 10)
    );
...
`;
  ...
  if (!s.includes('const resetFilters=')) {
    throw new Error('resetFilters anchor not found');
  }
...
if (!s.includes('Export PDF') && s.includes(toolbarAnchor)) {
  const toolbar = String.raw`    <section className="rounded-2xl border border-blue-400/20 bg-slate-900/80 p-3.5 shadow-xl sm:p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="text-[10px] font-black uppercase tracking-[.16em] text-blue-300">Export Data</div>
          <div className="mt-1 text-[11px] text-slate-300">Format standar • A4 Landscape • tabel presisi • \${exportRows.length} data mengikuti filter aktif.</div>
        </div>
...
s = s.replace(
  'Search, Trophy, ShieldCheck, Users, Filter, ChevronDown, Info, RefreshCw, Plus, Pencil, Trash2, X, Save, SlidersHorizontal',
  'Search, Trophy, ShieldCheck, Users, Filter, ChevronDown, Info, RefreshCw, Plus, Pencil, Trash2, X, Save, SlidersHorizontal, FileDown, FileSpreadsheet'
);

fs.writeFileSync(file, s, 'utf8');
console.log('[patch-seeded-export-layout] precise PDF/Excel export layout applied safely');
