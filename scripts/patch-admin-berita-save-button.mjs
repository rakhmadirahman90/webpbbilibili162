...
// The berita table currently has one image column (gambar_url). Keep the
// existing public schema and persist the carousel as newline-separated URLs.
const helperMarker = 'const parseBeritaImageUrls =';
if (!component.includes(helperMarker)) {
  const anchor = 'export default function AdminBerita({ session }: { session?: any }) {';
  const helper = `const parseBeritaImageUrls = (value?: string | null) =>
...
`;
  if (component.includes(anchor)) component = component.replace(anchor, helper + anchor);
}

// Make the main crop upload deterministic, public, and safe for repeated edits.
component = component.replace(
  /  const uploadProcessedImage = async \(\) => \{[\s\S]*?\n  \};\n\n  const uploadDirectFile = async/m,
...
component = component.replace(
  /  const uploadDirectFile = async \(file: File, field: 'gambar_url_2' \| 'gambar_url_3' \| 'gambar_url_4' \| 'gambar_url_5'\) => \{[\s\S]*?\n  \};\n\n  const handleSubmit/m,
`  const uploadDirectFile = async (file: File, field: 'gambar_url_2' | 'gambar_url_3' | 'gambar_url_4' | 'gambar_url_5') => {
    ...
    if (file.size > 8 * 1024 * 1024) {
      setFormError('Ukuran setiap gambar maksimal 8 MB.');
      return;
    }
...
// Replace the old background/optimistic save with an awaited, error-aware save.
component = component.replace(
...
// Edit existing records from the single DB image field without losing carousel items.
component = component.replace(
  /  const openModal = \(item\?: Berita\) => \{[\s\S]*?\n  \};\n\n  const closeModal/m,
...
// Make the primary image preview/list parsing explicit and resilient.
component = component.replace(
  /\(item\.gambar_url \|\| ''\)\.split\(\/\[\\\\s,\]\+\/\)\[0\]/g,
  "parseBeritaImageUrls(item.gambar_url)[0] || ''"
);
component = component.replace(
  /\(formData\.gambar_url \|\| ''\)\.split\(\/\[\\\\s,\]\+\/\)\[0\]/g,
  "parseBeritaImageUrls(formData.gambar_url)[0] || ''"
);

// Elegant, flexible save bar: it remains inside the form so Enter still submits,
// but is visually separated and thumb-friendly on mobile.
...
if (!component.includes('parseBeritaImageUrls(formData.gambar_url)[0]')) {
  console.warn('[patch-admin-berita-save-button] image preview matcher not replaced; keeping existing split fallback');
}
...
const css = `
${marker}
/* Professional mobile/tablet save action for Admin Berita. */
#admin-berita-page .fixed.z-\\[100\\] form {
  scroll-padding-bottom: 8rem !important;
}

@media (max-width: 767px) {
  #admin-berita-page .fixed.z-\\[100\\] {
    padding: max(.35rem, env(safe-area-inset-top)) .35rem max(.35rem, env(safe-area-inset-bottom)) !important;
  }

  #admin-berita-page .fixed.z-\\[100\\] > div {
    width: 100% !important;
  ...
  #admin-berita-page .fixed.z-\\[100\\] form > .sticky {
    position: sticky !important;
    bottom: 0 !important;
    z-index: 99999 !important;
  }

  #admin-berita-page .fixed.z-\\[100\\] form > .sticky button[type="submit"] {
  ...
  #admin-berita-page .fixed.z-\\[100\\] form input,
  #admin-berita-page .fixed.z-\\[100\\] form select,
  #admin-berita-page .fixed.z-\\[100\\] form textarea {
...
console.log('[patch-admin-berita-save-button] Supabase image persistence + responsive save UX applied');
