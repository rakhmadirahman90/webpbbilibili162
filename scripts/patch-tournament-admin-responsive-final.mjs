...
const marker = '/* __TOURNAMENT_ADMIN_RESPONSIVE_FINAL_V1__ */';
if (src.includes(marker)) {
  console.log('[patch-admin-responsive-final] already applied');
  process.exit(0);
}

// Final layout: one normal page scroll owned by AdminLayout, no nested table scroll on desktop,
// proportional columns, visible actions, and a dedicated mobile card layout.
const css = `<style>{\\`\n${marker}\n
...
.tournament-admin-page > div > header {
  ...
  border: 1px solid #17355b !important;
...
.tournament-admin-page > div > header > div:first-child {
  ...
  background: linear-gradient(110deg,#061224 0%,#0b2342 52%,#071326 100%) !important;
}
...
.tournament-admin-page > div > header > div:last-child span { color:#91a8c3 !important; }
.tournament-admin-page > div > header > div:last-child div > div:last-child { color:#f8fafc !important; }
/* Filter */
.tournament-admin-page > div > section:not(:last-child) {
  flex: none !important;
  border: 1px solid #17355b !important;
  background: #071326 !important;
  box-shadow: none !important;
}
.tournament-admin-page > div > section:not(:last-child) > div {
  display: grid !important;
  grid-template-columns: minmax(260px,1fr) repeat(3,minmax(150px,1fr)) 96px !important;
  gap: 8px !important;
}
.tournament-admin-page > div > section:not(:last-child) input,
.tournament-admin-page > div > section:not(:last-child) select,
.tournament-admin-page > div > section:not(:last-child) button {
  min-width: 0 !important;
...
/* Participant panel - normal document flow so AdminLayout can scroll when needed. */
...
.tournament-admin-page > div > section:last-child > div:first-child {
  ...
  border-color:#17355b !important;
  background:linear-gradient(90deg,#08172b,#0b1d35) !important;
}
...
/* Add participant button injected by this patch. */
.tournament-admin-page .participant-add-btn {
  ...
  box-shadow:0 6px 16px rgba(22,119,255,.22) !important;
}
/* Desktop table */
...
.tournament-admin-page table thead th {
  ...
  border-bottom:1px solid #17355b !important;
...
.tournament-admin-page table tbody tr {
  ...
  background:#071326 !important;
  border-bottom:1px solid #132a47 !important;
}
...
.tournament-admin-page table td:nth-child(2)>div:first-child,.tournament-admin-page table td:nth-child(3)>div,.tournament-admin-page table td:nth-child(4)>div:first-child {
...
/* Actions: never clip; 3 columns when six actions exist, 4 columns when only four. */
...
/* Mobile/tablet: use cards, not a squeezed table. */
@media (max-width: 1023px) {
  ...
  .tournament-admin-page > div > section:last-child > .md\\:hidden article > div:first-child > div:nth-child(2) > div:first-child { color:#f8fafc !important; }
...
@media (max-width: 640px) {
  .tournament-admin-page { padding:8px 8px 16px !important; }
...
// Add the create action directly to the participant header so it cannot disappear with CSS patches.
...
// Give Actions a stable class so future styling remains scoped to this page.
src = src.replace('<div className={`flex ${full?\'w-full\':\'justify-end\'} flex-wrap gap-1.5`}>', '<div className={`participant-actions flex ${full?\'w-full\':\'justify-end\'} flex-wrap gap-1.5`}>');

fs.writeFileSync(path, src);
console.log('[patch-admin-responsive-final] rebuilt registration admin layout for normal page scrolling, proportional desktop table, visible actions, and responsive mobile cards');
