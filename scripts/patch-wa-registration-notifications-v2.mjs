...
`;
  ...
  // The upload-only patch rewrites submit() and uses a direct window.open().
  // Replace that final notification block rather than relying on the old inline message.
  const start=src.lastIndexOf('      const wa=form.whatsapp.replace(/\\D/g,""');
  const end=src.indexOf('    }catch(err:any){',start);
  if(start<0||end<0)throw new Error('[patch-wa-v2] Final registration WA block not found.');
