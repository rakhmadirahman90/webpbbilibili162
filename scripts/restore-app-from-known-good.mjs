...
// The production source on main is authoritative.
const target = 'src/App.tsx';
const app = fs.readFileSync(target, 'utf8');

if (!/const handleNavigate\s*=\s*\(sectionId: string/.test(app)) {
  throw new Error('[app-source-guard] Current src/App.tsx is missing handleNavigate. Refusing to build.');
}

const declarations = (app.match(/const handleNavigate\s*=\s*\(sectionId: string/g) || []).length;
if (declarations !== 1) {
