...
if (!src.includes('const categoryCounts = useMemo')) {
  if (!src.includes(countMarker)) throw new Error('Category count marker not found');
...
if (!src.includes('Ganda Putra CC Lokal Parepare</p>')) {
  if (!src.includes(headerMarker)) throw new Error('Header/filter marker not found');
  src = src.replace(headerMarker, statsBlock);
}

fs.writeFileSync(path, src, 'utf8');
console.log('[patch-public-accepted-category-counts] Category participant counters added for CC Lokal Parepare and AD/BC-/C+C Ajatappareng.');
