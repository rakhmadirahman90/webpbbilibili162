...
if (s.includes(oldDropdown)) {
  s = s.replace(oldDropdown, newDropdown);
} else if (!s.includes('const isAthleteMenu =')) {
  throw new Error('Navbar dropdown render anchor not found');
}
...
console.log('Mobile Atlet submenu repair applied: stable parent identity + forced dropdown detection.');
