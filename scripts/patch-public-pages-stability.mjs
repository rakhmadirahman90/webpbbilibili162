...
for (const route of routes) {
  ...
  const anchor = anchors.find((candidate) => app.includes(candidate));
  if (!anchor) throw new Error(`[public-stability] no safe route anchor for ${route}`);
...
console.log('[public-stability] build-safe hard-stop applied');
