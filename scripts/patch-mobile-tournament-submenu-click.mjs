...
if (handlerRegex.test(s)) {
  ...
  // The newer sponsorship navigation patch can already own this handler.
  // Do not fail the entire production build when that happens.
  console.warn('[patch-mobile-tournament-submenu-click] handler already patched by an earlier navigation patch; skipping handler replacement');
}
...
console.log('[patch-mobile-tournament-submenu-click] mobile tournament navigation + body portal sidebar applied safely');

// The standalone Sponsorship route intentionally bypasses Navbar, so provide
// the same always-visible Beranda affordance on desktop and mobile.
...
if (!sponsor.includes('aria-label="Kembali ke Beranda"')) {
  ...
  if (sponsor.includes(mainMarker)) {
    ...
    console.warn('[patch-mobile-tournament-submenu-click] sponsorship main marker not found; skipping icon patch');
  }
}
fs.writeFileSync(sponsorPath, sponsor, 'utf8');
console.log('[patch-mobile-tournament-submenu-click] sponsorship Beranda icon applied safely for desktop + mobile');
