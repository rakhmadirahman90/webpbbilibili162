...
if (!src.includes('const paymentDisplayLabel =')) {
  ...
  src = src.replace(marker, marker + "const paymentDisplayLabel = (v?: string) => statusPay(v) === 'terverifikasi' ? 'Lunas' : 'Belum Bayar';\n");
  changes++;
}
...
src = src.replace("options={['Semua','menunggu','terverifikasi']}", "options={['Semua','Belum Bayar','Lunas']}");
...
const newRowPay = `<td className="px-4 py-4"><div className="flex flex-col gap-1">{paymentDisplayLabel(row.status_pembayaran)==='Lunas'?badge('Lunas','green'):badge('Belum Bayar','amber')}<span className="whitespace-nowrap text-[10px] font-bold text-slate-500">{rupiah(row.biaya_pendaftaran)}</span></div></td>`;
if (src.includes(oldRowPay)) { src = src.replace(oldRowPay, newRowPay); changes++; }

src = src.replace("{ps==='terverifikasi'?badge('Pembayaran OK','green'):badge('Bayar Menunggu','amber')}", "{paymentDisplayLabel(row.status_pembayaran)==='Lunas'?badge('Lunas','green'):badge('Belum Bayar','amber')}");
...
const newPayFilter = "const pay=(v:unknown)=>{const x=norm(v);return x.includes('terver')||x.includes('lunas')||x.includes('diterima')?'Lunas':'Belum Bayar'};";
...
const newExportStatus = "Status_Pembayaran:(/terver|lunas|diterima/i.test(clean(r.status_pembayaran))?'Lunas':'Belum Bayar')";
