...
export default async function handler(req: any, res: any) {
  ...
  try {
    const endpoint = `${SUPABASE_URL}/rest/v1/gallery?id=eq.${encodeURIComponent(id)}&select=id,title,url,type,updated_at,created_at`;
    ...
    // The WhatsApp preview title MUST be the actual photo/activity title.
    // Never replace a valid title with the generic PB Bilibili label.
    ...
    const version = String(req.query?.v || '18').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 24) || '18';
    const detailUrl = `${PUBLIC_DOMAIN}/?gallery=${encodeURIComponent(gallery.id)}&share=v18`;
    const shareUrl = `${PUBLIC_DOMAIN}/api/share-galeri?id=${encodeURIComponent(gallery.id)}&v=${version}`;
    const cacheImage = image ? `${image}${image.includes('?') ? '&' : '?'}share=v18` : '';
